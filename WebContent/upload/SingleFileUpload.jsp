<%@ page contentType="text/html; charset=utf-8" import="core.ListKeyBean,java.util.List;"%>
...
<html>
  <head>
	...
	<script type="text/javascript">
		...
		function uploadCheckFile(){
			...
			var pos_category=url.indexOf("category");
			category=url.substring(pos_category+9,url.length);
     			showFloatingLayer();
				uploadSubmit("form1","${pageContext.request.contextPath}/upload/fileUpload.action?action=uploadFile&readMode=no&category="+category);
     		}
		}
		...
		$(function(){
			...
			if(pos_id!==-1){
				...
				type=url.substring(pos_type+5,pos_category-1);
				readMode=url.substring(pos_readMode+9,pos_type-1);
				category=url.substring(pos_category+9,pos_id-1);
				if(id!='')
				{
				...
				uploadSubmit("form1","${pageContext.request.contextPath}/upload/fileUpload.action?action=getFile&readMode=" + readMode+"&category="+category);
			}
		});
  ...
  <body>
  	...
  	<s:form id="form1" name="form1" method="post" enctype="multipart/form-data">
	    	...
	    	<s:hidden name="category" id="category"></s:hidden>
	    	<s:hidden name="saveFiles" id="saveFiles"> </s:hidden>
	    	<s:hidden name="readMode" id="readMode"></s:hidden>
	    <s:div id="controlsDiv">
			<table width="200" border="0" bordercolordark="#FFFFFF" bordercolorlight="cccccc" cellpadding="3" cellspacing="0">
			  <tr>
				<td align="center">
					<%
						List<ListKeyBean> aaaList=(List<ListKeyBean>)request.getAttribute("fileOptionLeftList");
						if(aaaList.size()==0){
					 %>
					 		<s:if test="type=='img'"><img src ='${pageContext.request.contextPath}/upload/images/sourceimg/<s:text name="Common.NoPhoto_Img"/>'/></s:if>
					 <%}else{ListKeyBean lkb=aaaList.get(aaaList.size()-1);%>
						 <s:if test="type=='img'">
						 	<s:if test="category=='mgr'">
							<a href="${pageContext.request.contextPath}/<%=lkb.getKey()%>" target="_blank">
								<img src ='${pageContext.request.contextPath}/<%=lkb.getKey()%>' width='100' height='100' border='0'/>
							</a>
							<br><a href="${pageContext.request.contextPath}/<%=lkb.getKey()%>" target="_blank"><%=lkb.getValue()%></a><br>
							</s:if>
						</s:if>
						<s:else><a href="${pageContext.request.contextPath}/<%=lkb.getKey()%>" target="_blank"><%=lkb.getValue()%></a><br><br></s:else>
						<script language='javascript'>
							var tmp=document.getElementById('saveFiles').value;
							var strarray=tmp.split('|');
							document.getElementById('saveFiles').value=strarray[strarray.length-1];
						</script>
					<%}%>
			...
			<s:if test="readMode=='no'">
				<td rowspan="2" align="left" style="display:none">
			...
			<s:if test="readMode=='no'">
			  <tr>
				<td align="right">
					...
					<br><br>
                    <button class="button_small btn_gray" style="width:90px; height:24px" title=<s:text name="Common.Upload_File"/> onclick="uploadCheckFile();"><s:text name="Common.Upload_File"/></button>

                    <button class="button_small btn_gray" style="width:90px; height:24px" title=<s:text name="Common.Upload_File_Done"/> onclick="deleteFile();"><s:text name="Common.Upload_File_Done"/></button>
				</td>
				</tr>
			</s:if>
			</table>
