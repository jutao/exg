...
<html>
<head>
<title><s:text name="Advertisements.Title.query" /></title>
...
<script src="<%=basePath%>edu/jt/exg/js/advertisements.js"></script>
	...
	<div id="container">
		...
		<div style="overflow: hidden; background-color: #FFF;">
			...
			<s:form id="form1" name="form1"
				action="AdvertisementsAction.action?action=query" namespace="/">
			...
			<%-- 				<li style="margin-right: 5px;">
							...
							<!-- <li style="margin-right: 5px;"><input id="create_parent"
								name="create_parent" type="button" class="btn_small btn_orange"
								value="创建" disabled="disabled">
							</li> -->
						</ul>
					</div>
				</div>
						...
						<div class="grid_12">
							<div class="widget_wrap tabby">
								<div class="widget_top">
									...
									<h6><s:text name="Advertisements_table"/></h6>
								</div>
								<div class="widget_content">
									<s:if test="advertisementsList.size()>=0">
										<table id="action_tbl" class="display">
											<thead>
												<tr>
													...
													<th><s:text name="Advertisements.Name" /></th>
													<th><s:text name="Advertisements.Url" /></th>
													<%-- <th><s:text name="Advertisements.Category" /></th> --%>
													<th><s:text name="Advertisements.Display" /></th>
													<th><s:text name="Advertisements.Invalid" /></th>
													<th><s:text name="Common.Control" /></th>
												</tr>
											</thead>
											<tbody>
												<s:iterator value="advertisementsList" status="stuts" id="R">
													<tr class="t">
														...
														<td align="center"><a href="AdvertisementsAction.action?action=detail&id=<s:property value="#R.id"/>" 
															target="_self"><s:property value="#R.name" /></a></td>
														<td align="center"><s:property value="#R.url" /></td>
														...
														<td align="center">
															<a href="AdvertisementsAction.action?action=detail&id=<s:property value="#R.id"/>" target="_self"><img src='<%=basePath%>edu/jt/exg/images/detail.png' border='0' title=<s:text name="Common.Detail"/>></a>
						                                    <a href="AdvertisementsAction.action?action=initupdate&id=<s:property value="#R.id"/>" target="_self"><img src='<%=basePath%>edu/jt/exg/images/update.png' border='0' title=<s:text name="Common.Modify"/>></a>
															<a href="AdvertisementsAction.action?action=delete&id=<s:property value="#R.id"/>" target="_self"><img src ='<%=basePath%>edu/jt/exg/images/del.png' border='0' title=<s:text name="Common.Delete"/> onclick="return rusure('<s:text name="Common.Alert_Del"/>');"/></a>
														</td>
													</tr>
												</s:iterator>
											</tbody>
	...
	$(function() {
		...
		$("#reset").click(function() {
			window.setTimeout(function() {
				window.location = '<%=basePath%>AdvertisementsAction.action?action=initquery';
				return true;
			}, 0);
		});
		...
		$("#create_parent").click(function() {
			window.setTimeout(function() {
				window.location = '<%=basePath%>AdvertisementsAction.action?action=initcreate'; 
				return true;
			}, 0);
		});

		$("#delete").click(function() {
			getDeleteAllIds('<%=basePath%>AdvertisementsAction.action?action=deleteSelected&ids=',
