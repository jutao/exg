...
<html>
<head>
<title><s:text name="V_order_task.Title.query" /></title>
	...
	<div id="container">
		...
		<div style="overflow: hidden; background-color: #FFF;">
			...
			<s:form id="form1" name="form1"
				action="V_order_taskAction.action?action=query" namespace="/">
						...
						<div class="grid_12">
							<div class="widget_wrap tabby">
								<div class="widget_top">
									...
									<h6><s:text name="V_order_task_table"/></h6>
								</div>
								<div class="widget_content">
									<s:if test="v_order_taskList.size()>=0">
										<table id="action_tbl" class="display">
											<thead>
												<tr>
													...
													<th><s:text name="V_order_task.Taskkey" /></th>
													<th><s:text name="V_order_task.Orderkey" /></th>
													<th><s:text name="V_order_task.Req_name" /></th>
													<th><s:text name="V_order_task.Req_id" /></th>
													<th><s:text name="V_order_task.User_id" /></th>
													<th><s:text name="V_order_task.User_name" /></th>
													<th><s:text name="V_order_task.Invalid" /></th>
													<th><s:text name="Common.Control" /></th>
												</tr>
											</thead>
											<tbody>
												<s:iterator value="v_order_taskList" status="stuts" id="R">
													<tr class="gradeA">
														...
														<td align="center"><s:property value="#R.orderkey" /></td>		
														<td align="center"><a 
																href="V_order_taskAction.action?action=detail&id=<s:property value="#R.id"/>" 
																target="_self"><s:property value="#R.req_name" /></a></td>		
														...
														<td align="center">
															<a href="V_order_taskAction.action?action=detail&id=<s:property value="#R.id"/>" target="_self"><img src='<%=basePath%>edu/jt/exg/images/detail.png' border='0' title=<s:text name="Common.Detail"/>></a>
														</td>
													</tr>
												</s:iterator>
											</tbody>
	...
	$(function() {
		...
		$("#reset").click(function() {
			window.setTimeout(function() {
				window.location = '<%=basePath%>V_order_taskAction.action?action=initquery';
				return true;
			}, 0);
		});
		$("#create_parent").click(function() {
			window.setTimeout(function() {
				window.location = '<%=basePath%>V_order_taskAction.action?action=initcreate'; 
				return true;
			}, 0);
		});
		$("#delete").click(function() {
			getDeleteAllIds('<%=basePath%>CategoryAction.V_order_taskAction?action=deleteSelected&ids=',
