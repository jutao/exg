...
<html>
<head>
<title><s:text name="Categorymaster.Title.query" /></title>
...
<script src="<%=basePath%>edu/jt/exg/js/category_master.js"></script>
	...
	<div id="container">
		<div style="overflow: hidden; background-color: #FFF;">
			...
			<s:form id="form1" name="form1"
				action="Category_masterAction.action?action=query" namespace="/">

				<div class="page_title gray_sai">
					<div class="top_search" style="position:absolute;z-index:1000;">
						<ul id="search_box">
							<li style="margin-right: 5px;"><s:textfield id="parent_code" 
									name="parent_code" cssClass="search_input" style="width:100px;"
									placeholder="请输入母区分" /></li>
						...
						<div class="grid_12">
							<div class="widget_wrap tabby">
								<div class="widget_top">
									...
									<h6><s:text name="Categorymaster_table"/></h6>
								</div>
								<div class="widget_content">
								<s:set id="showPage" name="showPage" value="ShowPage"/>
									<s:if test="category_masterList.size()>=0">
										<table id="action_tbl" class="display">
											<thead>
												<tr>
													...
													<th><s:text name="Category_master.Parent_code" /></th>
													<th><s:text name="Category_master.Code" /></th>
													<th><s:text name="Category_master.Name" /></th>
													<th><s:text name="Category_master.Display" /></th>
													<th><s:text name="Category_master.Invalid" /></th>
													<th><s:text name="Common.Control" /></th>
												</tr>
											</thead>
											<tbody>
												<s:iterator value="category_masterList" status="stuts" id="R">
													<tr class="gradeA">
														...
														<td align="center"><s:property value="#R.parent_code" /></td>
														<td align="center"><s:property value="#R.code" /></td>
														<td align="center"><a 
																href="Category_masterAction.action?action=detail&id=<s:property value="#R.id"/>" 
														...
														<td align="center">
															<a href="Category_masterAction.action?action=detail&id=<s:property value="#R.id"/>" target="_self"><img src='<%=basePath%>edu/jt/exg/images/detail.png' border='0' title=<s:text name="Common.Detail"/>></a>
						                                    <a href="Category_masterAction.action?action=initupdate&id=<s:property value="#R.id"/>" target="_self"><img src='<%=basePath%>edu/jt/exg/images/update.png' border='0' title=<s:text name="Common.Modify"/>></a>
															<a href="Category_masterAction.action?action=delete&id=<s:property value="#R.id"/>" target="_self"><img src ='<%=basePath%>edu/jt/exg/images/del.png' border='0' title=<s:text name="Common.Delete"/> onclick="return rusure('<s:text name="Common.Alert_Del"/>');"/></a>
														</td>
													</tr>
												</s:iterator>
											</tbody>
	...
	$(function() {
		...
		$("#reset").click(function() {
			window.setTimeout(function() {
				window.location = '<%=basePath%>Category_masterAction.action?action=initquery';
				return true;
			}, 0);
		});
		...
		$("#create_parent").click(function() {
			window.setTimeout(function() {
				window.location = '<%=basePath%>Category_masterAction.action?action=initcreate'; 
				return true;
			}, 0);
		});
		...
		$("#delete").click(function() {
			getDeleteAllIds('<%=basePath%>Category_masterAction.action?action=deleteSelected&ids=',
