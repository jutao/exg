...
<html>
<head>
<title><s:text name="V_comment.Title.query" /></title>
...
<script	src="<%=basePath%>edu/jt/exg//js/jquery.multiFieldExtender.min.js"></script>
...
<script src="<%=basePath%>edu/jt/exg//js/v_comment.js"></script>
...
$("#search").click(function() {
		...
		$("#reset").click(function() {
			window.setTimeout(function() {
				window.location = '<%=basePath%>V_commentAction.action?action=initquery';
				return true;
			}, 0);
		});
		$("#query_child").click(function(){
			if($('#action_tbl .tr_select input:checked').length==1){
				...
				window.setTimeout(function() {
					window.location = '<%=basePath%>V_commentAction.action?action=query&parent_code='+parent_code;
					return true;
				}, 0);
			...
			getDeleteAllIds('<%=basePath%>V_commentAction.action?action=deleteSelected&ids=',
	...
	<div id="container">
		<div style="overflow: hidden; background-color: #FFF;">
			...
			<s:form id="form1" name="form1"
				action="V_commentAction.action?action=query" namespace="/">
					...
					<div class="top_search"style=" position:absolute;z-index:1000;">
						<ul id="search_box">
							<li style="margin-right: 5px;"><s:textfield id="user_id"
									name="user_id" cssClass="search_input1" style="width:100px;"
									placeholder="请输入用户id" /></li>
							<li style="margin-right: 5px;"><s:textfield id="target_id"
									name="target_id" cssClass="search_input1" style="width:100px;"
									placeholder="请输入标的id" /></li>
						...
						<div class="grid_12">
							<div class="widget_wrap tabby">
								<div class="widget_top">
									...
									<h6><s:text name="V_comment_table"/></h6>
								</div>
								<div class="widget_content">
									<s:if test="v_commentList.size()>=0">
										<table id="action_tbl" class="display">
											<thead>
												<tr>
													...
													<th><s:text name="V_comment.Userkey" /></th>
													<th><s:text name="V_comment.Targetkey" /></th>
													<th><s:text name="V_comment.Detail" /></th>
													<th><s:text name="V_comment.User_id" /></th>
													<th><s:text name="V_comment.Neckname" /></th>
													<th><s:text name="V_comment.Target_id" /></th>
													<th><s:text name="V_comment.Target_name" /></th>
													<th><s:text name="V_comment.Category" /></th>
													<th><s:text name="V_comment.Display" /></th>
													<th><s:text name="V_comment.Invalid" /></th>
													<th><s:text name="Common.Control" /></th>
												</tr>
											</thead>
											<tbody>
												<s:iterator value="v_commentList" status="stuts" id="R">
													<tr class="t">
														...
														<td align="center"><a 
															href="V_commentAction.action?action=detail&id=<s:property value="#R.id"/>" 
															target="_self"><s:property value="#R.userkey" /></a></td>
														<td align="center"><s:property value="#R.targetkey" /></td>
														<td align="center"><s:property value="#R.detail" /></td>
														<td align="center"><s:property value="#R.user_id" /></td>
														<td align="center"><s:property value="#R.neckname" /></td>
														<td align="center"><s:property value="#R.target_id" /></td>
														<td align="center"><s:property value="#R.target_name" /></td>
														...
														<td align="center">
															<a href="V_commentAction.action?action=detail&id=<s:property value="#R.id"/>" target="_self"><img src='<%=basePath%>edu/jt/exg//images/detail.png' border='0' title=<s:text name="Common.Detail"/>></a>
														</td>
													</tr>
												</s:iterator>
											</tbody>
