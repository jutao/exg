...
<html>
<head>
<title><s:text name="Withdrawals_detail.Title.query" /></title>
...
<script src="<%=basePath%>edu/jt/exg//js/withdrawals_detail.js"></script>
	...
	$(function() {
		...
		$("#reset").click(function() {
			window.setTimeout(function() {
				window.location = '<%=basePath%>Withdrawals_detailAction.action?action=initquery';
				return true;
			}, 0);
		});
		$("#create_parent").click(function() {
			window.setTimeout(function() {
				window.location = '<%=basePath%>Withdrawals_detailAction.action?action=initcreate'; 
				return true;
			}, 0);
		});
		$("#update_parent").click(function(){
	  		if($('#action_tbl .tr_select input:checked').length==1){
				...
				window.setTimeout(function() {
					window.location = '<%=basePath%>Withdrawals_detailAction.action?action=initupdate&id='+id;
					return true;
				}, 0);
			}else{
				alert("选择一个用户ID!");
		...
		$("#create_child").click(function(){
			if($('#action_tbl .tr_select input:checked').length==1){
				...
				window.setTimeout(function() {
					window.location = '<%=basePath%>Withdrawals_detailAction.action?action=initcreate&parent_code='+parent_code;
					return true;
				}, 0);
		...
		$("#query_child").click(function(){
			if($('#action_tbl .tr_select input:checked').length==1){
				...
				window.setTimeout(function() {
					window.location = '<%=basePath%>Withdrawals_detailAction.action?action=query&parent_code='+parent_code;
					return true;
				}, 0);
		...
		$("#delete").click(function() {
			getDeleteAllIds('<%=basePath%>Withdrawals_detailAction.action?action=deleteSelected&ids=',
	...
	<div id="container">
		<div style="overflow: hidden; background-color: #FFF;">
			...
			<s:form id="form1" name="form1"
				action="Withdrawals_detailAction.action?action=query" namespace="/">
					<div class="page_title gray_sai">
					<div class="top_search" style="position:absolute;z-index:1000;">
						<ul id="search_box">
							<li style="margin-right: 5px;"><s:textfield id="chargeid"
									name="chargeid" cssClass="search_input" style="width:100px;"
									placeholder="请输入提现流水号" /></li>
						...
						<div class="grid_12">
							<div class="widget_wrap tabby">
								<div class="widget_top">
									...
									<h6><s:text name="Withdrawals_detail_table"/></h6>
								</div>
								<div class="widget_content">
									<s:if test="withdrawals_detailList.size()>=0">
										<table id="action_tbl" class="display">
											<thead>
												<tr>
													...
													<th><s:text name="Withdrawals_detail.Chargeid" /></th>
													<th><s:text name="Withdrawals_detail.Userkey" /></th>
													<th><s:text name="Withdrawals_detail.Amount" /></th>
													<th><s:text name="Withdrawals_detail.Status" /></th>
													<th><s:text name="Withdrawals_detail.Invalid" /></th>
													<th><s:text name="Common.Control" /></th>
												</tr>
											</thead>
											<tbody>
												<s:iterator value="withdrawals_detailList" status="stuts" id="R">
													<tr class="t">
														...
														<td align="center"><a 
															href="Withdrawals_detailAction.action?action=detail&id=<s:property value="#R.id"/>" 
															target="_self"><s:property value="#R.chargeid" /></a></td>
														<td align="center"><s:property value="#R.userkey" /></td>
														<td align="center"><s:property value="#R.amount" /></td>
														<td align="center"><s:property value="#R.status" /></td>
														<td align="center"><s:property value="#R.invalid" /></td>
														<td align="center">
															<a href="Withdrawals_detailAction.action?action=detail&id=<s:property value="#R.id"/>" target="_self"><img src='<%=basePath%>edu/jt/exg//images/detail.png' border='0' title=<s:text name="Common.Detail"/>></a>
						                                    <a href="Withdrawals_detailAction.action?action=initupdate&id=<s:property value="#R.id"/>" target="_self"><img src='<%=basePath%>edu/jt/exg//images/update.png' border='0' title=<s:text name="Common.Modify"/>></a>
															<a href="Withdrawals_detailAction.action?action=delete&id=<s:property value="#R.id"/>" target="_self"><img src ='<%=basePath%>edu/jt/exg//images/del.png' border='0' title=<s:text name="Common.Delete"/> onclick="return rusure('<s:text name="Common.Alert_Del"/>');"/></a>
														</td>
													</tr>
												</s:iterator>
											</tbody>
