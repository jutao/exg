...
public class UserBatisDao {
	...
	public UserBatisDao(){super();}
	
	/**
	 * 得到IUser接口实例
	 * @return
	 */
	private IUser getIUser(){return (IUser)utility.BatisUtility.ctxProxool.getBean("userMapper");}
	
	/**
	 * 查询所有记录
	 * @return
	 */
	public List<UserBean> findByAll(){return getIUser().findByAll();}
	 ...
	 * @return
	 */
	public List<UserBean> findByAll(LinkedHashMap<String, String> orderByMap,String start,String range){
	 ...
	 * @return long
	 */
	public long findByAllCount(){return getIUser().findByAllCount().longValue();}
	 ...
	 * @return
	 */
	public UserBean findByKey(String key){return getIUser().findByKey(key);}
	 ...
	 * @return
	 */
	public List<UserBean> finder(LinkedHashMap<String, String> orderByMap,String start,String range,UserBean queryConditionsBean){
	 ...
	 * @return
	 */
	public List<UserBean> finder(LinkedHashMap<String, String> orderByMap,UserBean queryConditionsBean){
		StringBuffer sql=new StringBuffer("select * from "); 
		sql.replace(0, sql.length(), getWhereString(queryConditionsBean,sql.toString()));
		sql.replace(0, sql.length(), Utilities.getOrderByString(orderByMap,sql.toString()));
        return getIUser().finder(sql.toString());
	 ...
	 * @return
	 */
	public List<UserBean> finderEqual(LinkedHashMap<String, String> orderByMap,UserBean queryConditionsBean){
		StringBuffer sql=new StringBuffer("select * from ");
		sql.replace(0, sql.length(), getEqualWhereString(queryConditionsBean,sql.toString()));
		sql.replace(0, sql.length(), Utilities.getOrderByString(orderByMap,sql.toString()));
        return getIUser().finder(sql.toString());
	 ...
	 * @return
	 */
	public List<UserBean> finderEqual(LinkedHashMap<String, String> orderByMap,String start,String range,UserBean queryConditionsBean){
		StringBuffer sql=new StringBuffer("select * from ");
		sql.replace(0, sql.length(), getEqualWhereString(queryConditionsBean,sql.toString()));
		sql.replace(0, sql.length(), Utilities.getOrderByString(orderByMap,sql.toString()));
		try{sql.replace(0, sql.length(), SplitPage.getSqlPage(sql.toString(), Integer.parseInt(start), Integer.parseInt(range), ConstSetup.DATABASE));
	...
	public int deleteByKeys(List<String> keys){
		Integer result=new Integer(1);
		try{getIUser().deleteByKeys(keys);
	...
	public int updateByKey(UserBean userBean){
		Integer result=new Integer(1);
		try{getIUser().updateByKey(userBean);
		}catch(Exception e){result=0;}
		return result.intValue();
	}
	...
	public int insert(UserBean userBean){
		Integer result=new Integer(1);
		try{userBean.setId(Utilities.getRandomPK(true));
	 ...
	 * @return String sql语句
	 */
	private static String getEqualWhereString(UserBean queryConditionsBean,String sql){
		StringBuffer s=new StringBuffer(sql).append(ConstSetup.DATABASE_OWNER).append(".").append(TABLE).append(" where 1=1 and ");
		...
		if(queryConditionsBean.getNickname().trim().length()>0)
			s.append("nickname = '").append(Utilities.percentMarkFilter(queryConditionsBean.getNickname().trim())).append("' and ");
		...
		if(queryConditionsBean.getId_number().trim().length()>0)
			s.append("id_number = '").append(Utilities.percentMarkFilter(queryConditionsBean.getId_number().trim())).append("' and ");
		if(queryConditionsBean.getEmail().trim().length()>0)
			s.append("email = '").append(Utilities.percentMarkFilter(queryConditionsBean.getEmail().trim())).append("' and ");
		if(queryConditionsBean.getGender().trim().length()>0)
			s.append("gender = '").append(Utilities.percentMarkFilter(queryConditionsBean.getGender().trim())).append("' and ");
		if(queryConditionsBean.getAddress_province().trim().length()>0)
			s.append("address_province = '").append(Utilities.percentMarkFilter(queryConditionsBean.getAddress_province().trim())).append("' and ");
		if(queryConditionsBean.getAddress_city().trim().length()>0)
			s.append("address_city = '").append(Utilities.percentMarkFilter(queryConditionsBean.getAddress_city().trim())).append("' and ");
		if(queryConditionsBean.getAddress_area().trim().length()>0)
			s.append("address_area = '").append(Utilities.percentMarkFilter(queryConditionsBean.getAddress_area().trim())).append("' and ");
		if(queryConditionsBean.getAddress_details().trim().length()>0)
			s.append("address_details = '").append(Utilities.percentMarkFilter(queryConditionsBean.getAddress_details().trim())).append("' and ");
		if(queryConditionsBean.getHometown().trim().length()>0)
			s.append("hometown = '").append(Utilities.percentMarkFilter(queryConditionsBean.getHometown().trim())).append("' and ");
		if(queryConditionsBean.getOccupation().trim().length()>0)
			s.append("occupation = '").append(Utilities.percentMarkFilter(queryConditionsBean.getOccupation().trim())).append("' and ");
		if(queryConditionsBean.getInterest().trim().length()>0)
			s.append("interest = '").append(Utilities.percentMarkFilter(queryConditionsBean.getInterest().trim())).append("' and ");
		if(queryConditionsBean.getBankname().trim().length()>0)
			s.append("bankname = '").append(Utilities.percentMarkFilter(queryConditionsBean.getBankname().trim())).append("' and ");
		if(queryConditionsBean.getBank_branch().trim().length()>0)
			s.append("bank_branch = '").append(Utilities.percentMarkFilter(queryConditionsBean.getBank_branch().trim())).append("' and ");
		if(queryConditionsBean.getCard_number().trim().length()>0)
			s.append("card_number = '").append(Utilities.percentMarkFilter(queryConditionsBean.getCard_number().trim())).append("' and ");
		...
		if(queryConditionsBean.getRecommender_code().trim().length()>0)
			s.append("recommender_code = '").append(Utilities.percentMarkFilter(queryConditionsBean.getRecommender_code().trim())).append("' and ");
		...
		if(queryConditionsBean.getGesture_password().trim().length()>0)
			s.append("gesture_password = '").append(Utilities.percentMarkFilter(queryConditionsBean.getGesture_password().trim())).append("' and ");
		if(queryConditionsBean.getTrading_password().trim().length()>0)
			s.append("trading_password = '").append(Utilities.percentMarkFilter(queryConditionsBean.getTrading_password().trim())).append("' and ");
		...
		if(queryConditionsBean.getQualificat().trim().length()>0)
			s.append("qualificat = '").append(Utilities.percentMarkFilter(queryConditionsBean.getQualificat().trim())).append("' and ");
