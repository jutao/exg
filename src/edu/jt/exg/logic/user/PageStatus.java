...
public class PageStatus implements Serializable{
	...
	/**
	 * 设置查询页面返回记录的范围和排序规则
	 * @param UserAction action句柄
	 * @param params 设置参数
	 */
    public void setRecordsRangeAndOrderByRules(UserAction userAction,String params){
    	//获取记录范围*************************************************
        ...
        if(userAction.start==null) userAction.start="0";
        else if(userAction.start.length()==0) userAction.start="0";
        if(userAction.range==null) userAction.range=ConstSetup.REFERENCE_RANGE;
        else if(userAction.range.length()==0) userAction.range=ConstSetup.REFERENCE_RANGE;
	 ...
	 * @param params 设置参数
	 */
	public UserBean getQueryConditionsFromParams(UserAction userAction,String params){
		UserBean userBean=UserUtility.getIocHandel().getBean("UserBean",UserBean.class);
		String[] array=params.split("&");
		String tmp="";
		int pos=0;
		for(int i=0;i<array.length;i++){
			...
			userBean.setLevelFrom(Integer.valueOf(tmp.substring(pos+10,tmp.length())));
		}else if(array[i].indexOf("levelTo=")!=-1){
			pos=tmp.indexOf("levelTo=");
			userBean.setLevelTo(Integer.valueOf(tmp.substring(pos+8,tmp.length())));
		}
		...
		if(array[i].indexOf("address_province=")==0){
			...
			userBean.setAddress_province(tmp.substring(pos+17,tmp.length()));
		}
		...
		if(array[i].indexOf("address_details=")==0){
			...
			userBean.setAddress_details(tmp.substring(pos+16,tmp.length()));
		}
		...
		if(array[i].indexOf("balanceFrom=")==0){
			...
			userBean.setBalanceFrom(Double.valueOf(tmp.substring(pos+12,tmp.length())));
		}else if(array[i].indexOf("balanceTo=")==0){
			pos=tmp.indexOf("balanceTo=");
			userBean.setBalanceTo(Double.valueOf(tmp.substring(pos+10,tmp.length())));
		}
		if(array[i].indexOf("pointFrom=")==0){
			pos=tmp.indexOf("pointFrom=");
			userBean.setPointFrom(Integer.valueOf(tmp.substring(pos+10,tmp.length())));
		}else if(array[i].indexOf("pointTo=")!=-1){
			pos=tmp.indexOf("pointTo=");
			userBean.setPointTo(Integer.valueOf(tmp.substring(pos+8,tmp.length())));
		}
		if(array[i].indexOf("recommender_code=")==0){
			pos=tmp.indexOf("recommender_code=");
			userBean.setRecommender_code(tmp.substring(pos+17,tmp.length()));
		}
		...
		if(array[i].indexOf("gesture_password=")==0){
			...
			userBean.setGesture_password(tmp.substring(pos+17,tmp.length()));
		}
		if(array[i].indexOf("trading_password=")==0){
			pos=tmp.indexOf("trading_password=");
			userBean.setTrading_password(tmp.substring(pos+17,tmp.length()));
		}
		...
		if(array[i].indexOf("register_timeFrom=")==0){
			...
			userBean.setRegister_timeFrom(Utilities.transferStr2Timestamp(tmp,"yyyy-MM-dd hh:mm:ss"));
		}else if(array[i].indexOf("register_timeTo=")==0){
			pos=tmp.indexOf("register_timeTo=");
			tmp=tmp.substring(pos+16,tmp.length());
			userBean.setRegister_timeTo(Utilities.transferStr2Timestamp(tmp,"yyyy-MM-dd hh:mm:ss"));
		}
		if(array[i].indexOf("update_timeFrom=")==0){
			pos=tmp.indexOf("update_timeFrom=");
			tmp=tmp.substring(pos+16,tmp.length());
			userBean.setUpdate_timeFrom(Utilities.transferStr2Timestamp(tmp,"yyyy-MM-dd hh:mm:ss"));
		}else if(array[i].indexOf("update_timeTo=")==0){
			pos=tmp.indexOf("update_timeTo=");
			tmp=tmp.substring(pos+14,tmp.length());
			userBean.setUpdate_timeTo(Utilities.transferStr2Timestamp(tmp,"yyyy-MM-dd hh:mm:ss"));
		}
	...
	public UserBean getQueryConditionsFromForm(UserAction userAction){
		...
		userAction.register_timeFrom=Utilities.setTimeStampFunction(userAction.register_timeDateFrom,userAction.register_timeHourFrom,userAction.register_timeMinuteFrom);
		userBean.setRegister_timeFrom(userAction.register_timeFrom);
		userAction.register_timeTo=Utilities.setTimeStampFunction(userAction.register_timeDateTo,userAction.register_timeHourTo,userAction.register_timeMinuteTo);
		userBean.setRegister_timeTo(userAction.register_timeTo);
		userAction.update_timeFrom=Utilities.setTimeStampFunction(userAction.update_timeDateFrom,userAction.update_timeHourFrom,userAction.update_timeMinuteFrom);
		userBean.setUpdate_timeFrom(userAction.update_timeFrom);
		userAction.update_timeTo=Utilities.setTimeStampFunction(userAction.update_timeDateTo,userAction.update_timeHourTo,userAction.update_timeMinuteTo);
	...
	public String readControlsWrite2Datas(UserAction userAction){
		String errorMsg="";
				UserBean userBean=UserUtility.getIocHandel().getBean("UserBean",UserBean.class);
		if(userAction.action.equals("create")) userBean.setId(Utilities.getRandomPK(true));
		else userBean.setId(userAction.id);
		userBean=setUserBeanCommonAttributes(userBean,userAction);
		...
		userAction.batisDao=UserUtility.getIocHandel().getBean("UserBatisDao",UserBatisDao.class);
		if(userAction.action.equals("create")) errorMsg=Integer.toString(userAction.batisDao.insert(userBean));
		else errorMsg=Integer.toString(userAction.batisDao.updateByKey(userBean));
	...
	public void readDatasWrite2Controls(UserAction userAction){
		...
		write2ControlsCommon(userAction,userBean);
			...
			if(userBean.getLevel().intValue()>=0) userAction.setLevel(userBean.getLevel());
		if(userBean.getBalance()!=null)
			if(userBean.getBalance().doubleValue()!=0) userAction.setBalance(userBean.getBalance());
		if(userBean.getPoint()!=null)
			if(userBean.getPoint().intValue()>=0) userAction.setPoint(userBean.getPoint());
		//auto timestamp
		if(userAction.action.equals("detail")) userAction.setRegister_time(userBean.getRegister_time());//auto timestamp只作detail显示
		//auto timestamp
		if(userAction.action.equals("detail")) userAction.setUpdate_time(userBean.getUpdate_time());//auto timestamp只作detail显示
	 ...
	 * 保存查询条件写入跳转页面查询条件的控件
	 * @param UserAction action句柄
	 * @param UserBean action句柄
	 */
	public void readConditionsWrite2Controls(UserAction userAction,UserBean userBean){
		write2ControlsCommon(userAction,userBean);
				if(userBean.getLevelFrom()!=null)
			if(userBean.getLevelFrom().intValue()>=0) userAction.setLevelFrom(userBean.getLevelFrom());
		if(userBean.getLevelTo()!=null)
			if(userBean.getLevelTo().intValue()>=0) userAction.setLevelTo(userBean.getLevelTo());
		if(userBean.getBalanceFrom()!=null)
			if(userBean.getBalanceFrom().doubleValue()!=0) userAction.setBalanceFrom(userBean.getBalanceFrom());
		if(userBean.getBalanceTo()!=null)
			if(userBean.getBalanceTo().doubleValue()!=0) userAction.setBalanceTo(userBean.getBalanceTo());
		if(userBean.getPointFrom()!=null)
			if(userBean.getPointFrom().intValue()>=0) userAction.setPointFrom(userBean.getPointFrom());
		if(userBean.getPointTo()!=null)
			if(userBean.getPointTo().intValue()>=0) userAction.setPointTo(userBean.getPointTo());
		if(userBean.getRegister_timeFrom()!=null){
			userAction.setRegister_timeFrom(userBean.getRegister_timeFrom());
			userAction.setRegister_timeDateFrom(Utilities.csi.getSqlDate(userBean.getRegister_timeFrom()));
			cale.setTimeInMillis(userBean.getRegister_timeFrom().getTime());
			if(cale.get(Calendar.HOUR)<10) userAction.setRegister_timeHourFrom("0" + Integer.toString(cale.get(Calendar.HOUR)));
			else userAction.setRegister_timeHourFrom(Integer.toString(cale.get(Calendar.HOUR)));
			userAction.setRegister_timeHourSelectFrom(userAction.register_timeHourFrom);
			if(cale.get(Calendar.MINUTE)<10) userAction.setRegister_timeMinuteFrom("0" + Integer.toString(cale.get(Calendar.MINUTE)));
			else userAction.setRegister_timeMinuteFrom(Integer.toString(cale.get(Calendar.MINUTE)));
			userAction.setRegister_timeMinuteSelectFrom(userAction.register_timeMinuteFrom);
		}
		if(userBean.getRegister_timeTo()!=null){
			userAction.setRegister_timeTo(userBean.getRegister_timeTo());
			userAction.setRegister_timeDateTo(Utilities.csi.getSqlDate(userBean.getRegister_timeTo()));
			cale.setTimeInMillis(userBean.getRegister_timeFrom().getTime());
			if(cale.get(Calendar.HOUR)<10) userAction.setRegister_timeHourTo("0" + Integer.toString(cale.get(Calendar.HOUR)));
			else userAction.setRegister_timeHourTo(Integer.toString(cale.get(Calendar.HOUR)));
			userAction.setRegister_timeHourSelectTo(userAction.register_timeHourTo);
			if(cale.get(Calendar.MINUTE)<10) userAction.setRegister_timeMinuteTo("0" + Integer.toString(cale.get(Calendar.MINUTE)));
			else userAction.setRegister_timeMinuteTo(Integer.toString(cale.get(Calendar.MINUTE)));
			userAction.setRegister_timeMinuteSelectTo(userAction.register_timeMinuteTo);
		}
		if(userBean.getUpdate_timeFrom()!=null){
			userAction.setUpdate_timeFrom(userBean.getUpdate_timeFrom());
			userAction.setUpdate_timeDateFrom(Utilities.csi.getSqlDate(userBean.getUpdate_timeFrom()));
			cale.setTimeInMillis(userBean.getUpdate_timeFrom().getTime());
			if(cale.get(Calendar.HOUR)<10) userAction.setUpdate_timeHourFrom("0" + Integer.toString(cale.get(Calendar.HOUR)));
			else userAction.setUpdate_timeHourFrom(Integer.toString(cale.get(Calendar.HOUR)));
			userAction.setUpdate_timeHourSelectFrom(userAction.update_timeHourFrom);
			if(cale.get(Calendar.MINUTE)<10) userAction.setUpdate_timeMinuteFrom("0" + Integer.toString(cale.get(Calendar.MINUTE)));
			else userAction.setUpdate_timeMinuteFrom(Integer.toString(cale.get(Calendar.MINUTE)));
			userAction.setUpdate_timeMinuteSelectFrom(userAction.update_timeMinuteFrom);
		}
		if(userBean.getUpdate_timeTo()!=null){
			userAction.setUpdate_timeTo(userBean.getUpdate_timeTo());
			userAction.setUpdate_timeDateTo(Utilities.csi.getSqlDate(userBean.getUpdate_timeTo()));
			cale.setTimeInMillis(userBean.getUpdate_timeFrom().getTime());
			if(cale.get(Calendar.HOUR)<10) userAction.setUpdate_timeHourTo("0" + Integer.toString(cale.get(Calendar.HOUR)));
			else userAction.setUpdate_timeHourTo(Integer.toString(cale.get(Calendar.HOUR)));
			userAction.setUpdate_timeHourSelectTo(userAction.update_timeHourTo);
			if(cale.get(Calendar.MINUTE)<10) userAction.setUpdate_timeMinuteTo("0" + Integer.toString(cale.get(Calendar.MINUTE)));
			else userAction.setUpdate_timeMinuteTo(Integer.toString(cale.get(Calendar.MINUTE)));
			userAction.setUpdate_timeMinuteSelectTo(userAction.update_timeMinuteTo);
		}
	 ...
	 * @param type 控件类型
	 */
	public UserBean wrapperUserBean(UserAction userAction,UserBean userBean,String type){
				//上传照片显示***********************************************************************
		if(type.equals("show")) userBean.setIcon(Utilities.getWrapperUploadFileLinkString(userAction.request,userBean.getIcon(),"image"));
		else userBean.setIcon(userBean.getIcon());
		//s:select***************************************************************************
		InitPageControl.initGenderMap(userAction);
		userBean.setGender(Utilities.getWrapperSelect(userBean.getGender(),userAction.genderMap));
		//s:select***************************************************************************
		InitPageControl.initAddress_provinceMap(userAction);
		userBean.setAddress_province(Utilities.getWrapperSelect(userBean.getAddress_province(),userAction.address_provinceMap));
		//s:select***************************************************************************
		InitPageControl.initAddress_cityMap(userAction);
		userBean.setAddress_city(Utilities.getWrapperSelect(userBean.getAddress_city(),userAction.address_cityMap));
		//s:select***************************************************************************
		InitPageControl.initAddress_areaMap(userAction);
		userBean.setAddress_area(Utilities.getWrapperSelect(userBean.getAddress_area(),userAction.address_areaMap));
		//s:select***************************************************************************
		InitPageControl.initBanknameMap(userAction);
		userBean.setBankname(Utilities.getWrapperSelect(userBean.getBankname(),userAction.banknameMap));
		//s:select***************************************************************************
		InitPageControl.initCategoryMap(userAction);
		userBean.setCategory(Utilities.getWrapperSelect(userBean.getCategory(),userAction.categoryMap));
		//s:checkbox************************************************************************
		userBean.setUsertype(Utilities.getWrapperCheckbox(userBean.getUsertype(),"Y","N"));
		//上传照片显示***********************************************************************
		if(type.equals("show")) userBean.setQualificat(Utilities.getWrapperUploadFileLinkString(userAction.request,userBean.getQualificat(),"image"));
		else userBean.setQualificat(userBean.getQualificat());
		//s:select***************************************************************************
		InitPageControl.initInvalidMap(userAction);
    ...
    public List<UserBean> wrapperRecorders(UserAction userAction,String type){
    	UserBean user=null;
    	List<UserBean> newUserList=UserUtility.getIocHandel().getBean("List_UserBean",java.util.ArrayList.class);
	...
	private void write2ControlsCommon(UserAction userAction,UserBean userBean){
		...
		if(userBean.getUserid()!=null)
			if(userBean.getUserid().trim().length()>0) userAction.setUserid(userBean.getUserid());
		//textfield
		if(userBean.getName()!=null)
			if(userBean.getName().trim().length()>0) userAction.setName(userBean.getName());
		//textfield
		if(userBean.getNickname()!=null)
			if(userBean.getNickname().trim().length()>0) userAction.setNickname(userBean.getNickname());
		//img upload
		if(userBean.getIcon()!=null){
			if(userBean.getIcon().trim().length()>0) userAction.setIcon(userBean.getIcon());
			else userAction.setIcon("");
		}else userAction.setIcon("");
		//textfield
		if(userBean.getId_number()!=null)
			if(userBean.getId_number().trim().length()>0) userAction.setId_number(userBean.getId_number());
		//textfield
		if(userBean.getEmail()!=null)
			if(userBean.getEmail().trim().length()>0) userAction.setEmail(userBean.getEmail());
		//select
		if(userBean.getGender()!=null){
			if(userBean.getGender().trim().length()>0){
				...
				}else userAction.setGender(Utilities.getWrapperSelect(userBean.getGender(),userAction.genderMap));
			}
		}
		//select
		if(userBean.getAddress_province()!=null){
			if(userBean.getAddress_province().trim().length()>0){
				if(!userAction.action.equals("detail")){
					...
					userAction.setAddress_provinceSelect(userBean.getAddress_province());
				}else userAction.setAddress_province(Utilities.getWrapperSelect(userBean.getAddress_province(),userAction.address_provinceMap));
			}
		}
		//select
		if(userBean.getAddress_city()!=null){
			if(userBean.getAddress_city().trim().length()>0){
				if(!userAction.action.equals("detail")){
					...
					userAction.setAddress_citySelect(userBean.getAddress_city());
				}else userAction.setAddress_city(Utilities.getWrapperSelect(userBean.getAddress_city(),userAction.address_cityMap));
			}
		}
		//select
		if(userBean.getAddress_area()!=null){
			if(userBean.getAddress_area().trim().length()>0){
				if(!userAction.action.equals("detail")){
					...
					userAction.setAddress_areaSelect(userBean.getAddress_area());
				}else userAction.setAddress_area(Utilities.getWrapperSelect(userBean.getAddress_area(),userAction.address_areaMap));
			}
		}
		//textfield
		if(userBean.getAddress_details()!=null)
			if(userBean.getAddress_details().trim().length()>0) userAction.setAddress_details(userBean.getAddress_details());
		//textfield
		if(userBean.getHometown()!=null)
			if(userBean.getHometown().trim().length()>0) userAction.setHometown(userBean.getHometown());
		//textfield
		if(userBean.getOccupation()!=null)
			if(userBean.getOccupation().trim().length()>0) userAction.setOccupation(userBean.getOccupation());
		//textfield
		if(userBean.getInterest()!=null)
			if(userBean.getInterest().trim().length()>0) userAction.setInterest(userBean.getInterest());
		//select
		if(userBean.getBankname()!=null){
			if(userBean.getBankname().trim().length()>0){
				...
				}else userAction.setBankname(Utilities.getWrapperSelect(userBean.getBankname(),userAction.banknameMap));
			}
		}
		//textfield
		if(userBean.getBank_branch()!=null)
			if(userBean.getBank_branch().trim().length()>0) userAction.setBank_branch(userBean.getBank_branch());
		//textfield
		if(userBean.getCard_number()!=null)
			if(userBean.getCard_number().trim().length()>0) userAction.setCard_number(userBean.getCard_number());
		//textfield
		if(userBean.getRecommender_code()!=null)
			if(userBean.getRecommender_code().trim().length()>0) userAction.setRecommender_code(userBean.getRecommender_code());
		//textfield
		if(userBean.getLogin_password()!=null)
			if(userBean.getLogin_password().trim().length()>0) userAction.setLogin_password(userBean.getLogin_password());
		//textfield
		if(userBean.getGesture_password()!=null)
			if(userBean.getGesture_password().trim().length()>0) userAction.setGesture_password(userBean.getGesture_password());
		//textfield
		if(userBean.getTrading_password()!=null)
			if(userBean.getTrading_password().trim().length()>0) userAction.setTrading_password(userBean.getTrading_password());
		//select
		if(userBean.getCategory()!=null){
			if(userBean.getCategory().trim().length()>0){
				...
				}else userAction.setCategory(Utilities.getWrapperSelect(userBean.getCategory(),userAction.categoryMap));
			}
		}
		//checkbox
		...
		if(userBean.getQualificat()!=null){
			if(userBean.getQualificat().trim().length()>0) userAction.setQualificat(userBean.getQualificat());
			else userAction.setQualificat("");
		}else userAction.setQualificat("");
		//textfield
		if(userBean.getYubei1()!=null)
			if(userBean.getYubei1().trim().length()>0) userAction.setYubei1(userBean.getYubei1());
		//textfield
		if(userBean.getYubei2()!=null)
			if(userBean.getYubei2().trim().length()>0) userAction.setYubei2(userBean.getYubei2());
		//textfield
		if(userBean.getYubei3()!=null)
			if(userBean.getYubei3().trim().length()>0) userAction.setYubei3(userBean.getYubei3());
		//select
		if(userBean.getInvalid()!=null){
			if(userBean.getInvalid().trim().length()>0){
				...
				}else userAction.setInvalid(Utilities.getWrapperSelect(userBean.getInvalid(),userAction.invalidMap));
			}
		}
	 ...
	 * @param UserAction action句柄
	 */
	private UserBean setUserBeanCommonAttributes(UserBean userBean,UserAction userAction){
	...
	public String saveRequestParameters(UserBean queryConditionsBean){
		StringBuffer paramStr=new StringBuffer("");
				...
				paramStr.append("&levelFrom=").append(queryConditionsBean.getLevelFrom());
		if(queryConditionsBean.getLevelTo()!=null)
				paramStr.append("&levelTo=").append(queryConditionsBean.getLevelTo());
		...
		if(queryConditionsBean.getNickname().trim().length()>0)
			paramStr.append("&nickname=").append(queryConditionsBean.getNickname().trim());
		...
		if(queryConditionsBean.getId_number().trim().length()>0)
			paramStr.append("&id_number=").append(queryConditionsBean.getId_number().trim());
		if(queryConditionsBean.getEmail().trim().length()>0)
			paramStr.append("&email=").append(queryConditionsBean.getEmail().trim());
		if(queryConditionsBean.getGender()!=null)
			if(queryConditionsBean.getGender().trim().length()>0)
				paramStr.append("&gender=").append(queryConditionsBean.getGender().trim());
		if(queryConditionsBean.getAddress_province()!=null)
			if(queryConditionsBean.getAddress_province().trim().length()>0)
				paramStr.append("&address_province=").append(queryConditionsBean.getAddress_province().trim());
		if(queryConditionsBean.getAddress_city()!=null)
			if(queryConditionsBean.getAddress_city().trim().length()>0)
				paramStr.append("&address_city=").append(queryConditionsBean.getAddress_city().trim());
		if(queryConditionsBean.getAddress_area()!=null)
			if(queryConditionsBean.getAddress_area().trim().length()>0)
				paramStr.append("&address_area=").append(queryConditionsBean.getAddress_area().trim());
		if(queryConditionsBean.getAddress_details().trim().length()>0)
			paramStr.append("&address_details=").append(queryConditionsBean.getAddress_details().trim());
		if(queryConditionsBean.getHometown().trim().length()>0)
			paramStr.append("&hometown=").append(queryConditionsBean.getHometown().trim());
		if(queryConditionsBean.getOccupation().trim().length()>0)
			paramStr.append("&occupation=").append(queryConditionsBean.getOccupation().trim());
		if(queryConditionsBean.getInterest().trim().length()>0)
			paramStr.append("&interest=").append(queryConditionsBean.getInterest().trim());
		if(queryConditionsBean.getBankname()!=null)
			if(queryConditionsBean.getBankname().trim().length()>0)
				paramStr.append("&bankname=").append(queryConditionsBean.getBankname().trim());
		if(queryConditionsBean.getBank_branch().trim().length()>0)
			paramStr.append("&bank_branch=").append(queryConditionsBean.getBank_branch().trim());
		if(queryConditionsBean.getCard_number().trim().length()>0)
			paramStr.append("&card_number=").append(queryConditionsBean.getCard_number().trim());
		if(queryConditionsBean.getBalanceFrom()!=null)
				paramStr.append("&balanceFrom=").append(queryConditionsBean.getBalanceFrom());
		if(queryConditionsBean.getBalanceTo()!=null)
				paramStr.append("&balanceTo=").append(queryConditionsBean.getBalanceTo());
		if(queryConditionsBean.getPointFrom()!=null)
				paramStr.append("&pointFrom=").append(queryConditionsBean.getPointFrom());
		if(queryConditionsBean.getPointTo()!=null)
				paramStr.append("&pointTo=").append(queryConditionsBean.getPointTo());
		if(queryConditionsBean.getRecommender_code().trim().length()>0)
			paramStr.append("&recommender_code=").append(queryConditionsBean.getRecommender_code().trim());
		...
		if(queryConditionsBean.getGesture_password().trim().length()>0)
			paramStr.append("&gesture_password=").append(queryConditionsBean.getGesture_password().trim());
		if(queryConditionsBean.getTrading_password().trim().length()>0)
			paramStr.append("&trading_password=").append(queryConditionsBean.getTrading_password().trim());
		...
		if(queryConditionsBean.getQualificat().trim().length()>0)
			paramStr.append("&qualificat=").append(queryConditionsBean.getQualificat().trim());
