...
import edu.jt.exg.logic.v_order_task.InitPageControl;
...
public class V_order_taskAction extends V_order_taskAttributeAction {
	...
	public String execute() throws Exception {
		...
		ps=V_order_taskUtility.getIocHandel().getBean("PageStatus",PageStatus.class);
	...
	private String doCommonInitQuery(String parm){
		...
		batisDao=V_order_taskUtility.getIocHandel().getBean("V_order_taskBatisDao",V_order_taskBatisDao.class);
		
		
		orderByMap.put(V_order_taskBatisDao.ID_ASC, null);//sqlserver必须排序否则分页会有问题
		
		
		
		
		v_order_taskList=batisDao.findByAll(orderByMap,start,range);
		
    	v_order_taskList=ps.wrapperRecorders(this,"show");
    	
    	
    	long num=batisDao.findByAllCount();
    	
        String page=Page.getPage(singleServletModel.getRequest(),
        		new StringBuffer("action=").append(parm).append("&go2page=edu/jt/exg/action/V_order_taskAction.action").toString(),
        ...
        setStart(start);
        setV_order_taskList(v_order_taskList);
	...
	private String doCommonQuery(String parm){
		...
		if(params!=null){
			...
			v_order_task=ps.getQueryConditionsFromParams(this, params);
		}else{
			ps.setRecordsRangeAndOrderByRules(this);
			v_order_task=ps.getQueryConditionsFromForm(this);
		}
		...
		v_order_taskList=batisDao.finder(orderByMap,start,range,v_order_task);
		
		v_order_taskList=ps.wrapperRecorders(this,"show");
		
		
		long num=batisDao.finderCount(v_order_task);
		
		String page=Page.getPage(singleServletModel.getRequest(),
				new StringBuffer("action=").append(parm).append(ps.saveRequestParameters(v_order_task)).append("&go2page=edu/jt/exg/action/V_order_taskAction.action").toString(),
				Integer.parseInt(start),Integer.parseInt(range),num);
		showPage.setPage(page);
		String page1=Page.getPage1(singleServletModel.getRequest(),
				new StringBuffer("action=").append(parm).append(ps.saveRequestParameters(v_order_task)).append("&go2page=edu/jt/exg/action/V_order_taskAction.action").toString(),
        ...
        setShowPage(showPage);
		ps.setControlsValues(this);//控件初始化
		ps.readConditionsWrite2Controls(this,v_order_task);
		return parm;
	}
