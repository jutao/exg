...
import edu.jt.exg.logic.comment.InitPageControl;
import edu.jt.exg.logic.comment.PageStatus;
...
public class CommentAction extends CommentAttributeAction {
	...
	public String execute() throws Exception {
		...
		ps=CommentUtility.getIocHandel().getBean("PageStatus",PageStatus.class);
	...
	private String doCommonInitQuery(String parm){
		...
		batisDao=CommentUtility.getIocHandel().getBean("CommentBatisDao",CommentBatisDao.class);
		
		
		orderByMap.put(CommentBatisDao.ID_ASC, null);//sqlserver必须排序否则分页会有问题
		
		
		
		
		commentList=batisDao.findByAll(orderByMap,start,range);
		
    	commentList=ps.wrapperRecorders(this,"show");
    	
    	
    	long num=batisDao.findByAllCount();
    	
        String page=Page.getPage(singleServletModel.getRequest(),
        		new StringBuffer("action=").append(parm).append("&go2page=edu/jt/exg/action/CommentAction.action").toString(),
	...
	private String doCommonQuery(String parm){
		...
		if(params!=null){
			...
			comment=ps.getQueryConditionsFromParams(this, params);
		}else{
			ps.setRecordsRangeAndOrderByRules(this);
			comment=ps.getQueryConditionsFromForm(this);
		}
		...
		commentList=batisDao.finder(orderByMap,start,range,comment);
		
		commentList=ps.wrapperRecorders(this,"show");
		
		
		long num=batisDao.finderCount(comment);
		
		String page=Page.getPage(singleServletModel.getRequest(),
				new StringBuffer("action=").append(parm).append(ps.saveRequestParameters(comment)).append("&go2page=edu/jt/exg/action/CommentAction.action").toString(),
				Integer.parseInt(start),Integer.parseInt(range),num);
		showPage.setPage(page);
		String page1=Page.getPage1(singleServletModel.getRequest(),
				new StringBuffer("action=").append(parm).append(ps.saveRequestParameters(comment)).append("&go2page=edu/jt/exg/action/CommentAction.action").toString(),
        ...
        setShowPage(showPage);
		ps.setControlsValues(this);//控件初始化
		ps.readConditionsWrite2Controls(this,comment);
		return parm;
	}
