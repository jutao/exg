...
import edu.jt.exg.logic.consume_detail.InitPageControl;
import edu.jt.exg.logic.consume_detail.PageStatus;
import edu.jt.exg.logic.consume_detail.Consume_detailBean;


import edu.jt.exg.logic.consume_detail.Consume_detailBatisDao;

import edu.jt.exg.logic.consume_detail.Consume_detailUtility;



public class Consume_detailAction extends Consume_detailAttributeAction {
	...
	public String execute() throws Exception {
		...
		ps=Consume_detailUtility.getIocHandel().getBean("PageStatus",PageStatus.class);
	...
	private String doCommonInitQuery(String parm){
		...
		batisDao=Consume_detailUtility.getIocHandel().getBean("Consume_detailBatisDao",Consume_detailBatisDao.class);
		
		
		orderByMap.put(Consume_detailBatisDao.ID_ASC, null);//sqlserver必须排序否则分页会有问题
		
		
		
		
		consume_detailList=batisDao.findByAll(orderByMap,start,range);
		
    	consume_detailList=ps.wrapperRecorders(this,"show");
    	
    	
    	long num=batisDao.findByAllCount();
    	
        String page=Page.getPage(singleServletModel.getRequest(),
        		new StringBuffer("action=").append(parm).append("&go2page=edu/jt/exg/action/Consume_detailAction.action").toString(),
        ...
        setStart(start);
        setConsume_detailList(consume_detailList);
	...
	private String doCommonQuery(String parm){
		...
		if(params!=null){
			...
			consume_detail=ps.getQueryConditionsFromParams(this, params);
		}else{
			ps.setRecordsRangeAndOrderByRules(this);
			consume_detail=ps.getQueryConditionsFromForm(this);
		}
		...
		consume_detailList=batisDao.finder(orderByMap,start,range,consume_detail);
		
		consume_detailList=ps.wrapperRecorders(this,"show");
		
		
		long num=batisDao.finderCount(consume_detail);
		
		String page=Page.getPage(singleServletModel.getRequest(),
				new StringBuffer("action=").append(parm).append(ps.saveRequestParameters(consume_detail)).append("&go2page=edu/jt/exg/action/Consume_detailAction.action").toString(),
				Integer.parseInt(start),Integer.parseInt(range),num);
		showPage.setPage(page);
		String page1=Page.getPage1(singleServletModel.getRequest(),
				new StringBuffer("action=").append(parm).append(ps.saveRequestParameters(consume_detail)).append("&go2page=edu/jt/exg/action/Consume_detailAction.action").toString(),
        ...
        setShowPage(showPage);
		ps.setControlsValues(this);//控件初始化
		ps.readConditionsWrite2Controls(this,consume_detail);
		return parm;
	}
