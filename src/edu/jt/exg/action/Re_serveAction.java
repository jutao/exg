...
public class Re_serveAction extends Re_serveAttributeAction {
	...
	public String execute() throws Exception {
		...
		ps=Re_serveUtility.getIocHandel().getBean("PageStatus",PageStatus.class);
	...
	private String doCommonInitQuery(String parm){
		...
		orderByMap.put(Re_serveBatisDao.ID_ASC, null);//sqlserver必须排序否则分页会有问题
		
		
		
		
		re_serveList=batisDao.findByAll(orderByMap,start,range);
		
    	re_serveList=ps.wrapperRecorders(this,"show");
    	
    	
    	long num=batisDao.findByAllCount();
    	
        String page=Page.getPage(singleServletModel.getRequest(),
        		new StringBuffer("action=").append(parm).append("&go2page=edu/jt/exg/action/Re_serveAction.action").toString(),
	...
	private String doCommonQuery(String parm){
		...
		if(params!=null){
			...
			re_serve=ps.getQueryConditionsFromParams(this, params);
		}else{
			ps.setRecordsRangeAndOrderByRules(this);
			re_serve=ps.getQueryConditionsFromForm(this);
		}
		...
		re_serveList=batisDao.finder(orderByMap,start,range,re_serve);
		
		re_serveList=ps.wrapperRecorders(this,"show");
		
		
		long num=batisDao.finderCount(re_serve);
		
		String page=Page.getPage(singleServletModel.getRequest(),
				new StringBuffer("action=").append(parm).append(ps.saveRequestParameters(re_serve)).append("&go2page=edu/jt/exg/action/Re_serveAction.action").toString(),
				Integer.parseInt(start),Integer.parseInt(range),num);
		showPage.setPage(page);
		String page1=Page.getPage1(singleServletModel.getRequest(),
				new StringBuffer("action=").append(parm).append(ps.saveRequestParameters(re_serve)).append("&go2page=edu/jt/exg/action/Re_serveAction.action").toString(),
        ...
        setShowPage(showPage);
		ps.setControlsValues(this);//控件初始化
		ps.readConditionsWrite2Controls(this,re_serve);
		return parm;
	}
