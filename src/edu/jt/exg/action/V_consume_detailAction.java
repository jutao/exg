...
import edu.jt.exg.logic.v_consume_detail.InitPageControl;
...
import edu.jt.exg.logic.v_consume_detail.V_consume_detailUtility;



public class V_consume_detailAction extends V_consume_detailAttributeAction {
	...
	public String execute() throws Exception {
		...
		ps=V_consume_detailUtility.getIocHandel().getBean("PageStatus",PageStatus.class);
	...
	private String doCommonInitQuery(String parm){
		...
		batisDao=V_consume_detailUtility.getIocHandel().getBean("V_consume_detailBatisDao",V_consume_detailBatisDao.class);
		
		
		orderByMap.put(V_consume_detailBatisDao.ID_ASC, null);//sqlserver必须排序否则分页会有问题
		
		
		
		
		v_consume_detailList=batisDao.findByAll(orderByMap,start,range);
		
    	v_consume_detailList=ps.wrapperRecorders(this,"show");
    	
    	
    	long num=batisDao.findByAllCount();
    	
        String page=Page.getPage(singleServletModel.getRequest(),
        		new StringBuffer("action=").append(parm).append("&go2page=edu/jt/exg/action/V_consume_detailAction.action").toString(),
        ...
        setStart(start);
        setV_consume_detailList(v_consume_detailList);
	...
	private String doCommonQuery(String parm){
		...
		if(params!=null){
			...
			v_consume_detail=ps.getQueryConditionsFromParams(this, params);
		}else{
			ps.setRecordsRangeAndOrderByRules(this);
			v_consume_detail=ps.getQueryConditionsFromForm(this);
		}
		...
		v_consume_detailList=batisDao.finder(orderByMap,start,range,v_consume_detail);
		
		v_consume_detailList=ps.wrapperRecorders(this,"show");
		
		
		long num=batisDao.finderCount(v_consume_detail);
		
		String page=Page.getPage(singleServletModel.getRequest(),
				new StringBuffer("action=").append(parm).append(ps.saveRequestParameters(v_consume_detail)).append("&go2page=edu/jt/exg/action/V_consume_detailAction.action").toString(),
				Integer.parseInt(start),Integer.parseInt(range),num);
		showPage.setPage(page);
		String page1=Page.getPage1(singleServletModel.getRequest(),
				new StringBuffer("action=").append(parm).append(ps.saveRequestParameters(v_consume_detail)).append("&go2page=edu/jt/exg/action/V_consume_detailAction.action").toString(),
        ...
        setShowPage(showPage);
		ps.setControlsValues(this);//控件初始化
		ps.readConditionsWrite2Controls(this,v_consume_detail);
		return parm;
	}
