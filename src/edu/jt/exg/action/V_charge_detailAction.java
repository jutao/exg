...
import edu.jt.exg.logic.v_charge_detail.InitPageControl;
...
import edu.jt.exg.logic.v_charge_detail.V_charge_detailUtility;



public class V_charge_detailAction extends V_charge_detailAttributeAction {
	...
	public String execute() throws Exception {
		...
		ps=V_charge_detailUtility.getIocHandel().getBean("PageStatus",PageStatus.class);
	...
	private String doCommonInitQuery(String parm){
		...
		batisDao=V_charge_detailUtility.getIocHandel().getBean("V_charge_detailBatisDao",V_charge_detailBatisDao.class);
		
		
		orderByMap.put(V_charge_detailBatisDao.ID_ASC, null);//sqlserver必须排序否则分页会有问题
		
		
		
		
		v_charge_detailList=batisDao.findByAll(orderByMap,start,range);
		
    	v_charge_detailList=ps.wrapperRecorders(this,"show");
    	
    	
    	long num=batisDao.findByAllCount();
    	
        String page=Page.getPage(singleServletModel.getRequest(),
        		new StringBuffer("action=").append(parm).append("&go2page=edu/jt/exg/action/V_charge_detailAction.action").toString(),
        ...
        setStart(start);
        setV_charge_detailList(v_charge_detailList);
	...
	private String doCommonQuery(String parm){
		...
		if(params!=null){
			...
			v_charge_detail=ps.getQueryConditionsFromParams(this, params);
		}else{
			ps.setRecordsRangeAndOrderByRules(this);
			v_charge_detail=ps.getQueryConditionsFromForm(this);
		}
		...
		v_charge_detailList=batisDao.finder(orderByMap,start,range,v_charge_detail);
		
		v_charge_detailList=ps.wrapperRecorders(this,"show");
		
		
		long num=batisDao.finderCount(v_charge_detail);
		
		String page=Page.getPage(singleServletModel.getRequest(),
				new StringBuffer("action=").append(parm).append(ps.saveRequestParameters(v_charge_detail)).append("&go2page=edu/jt/exg/action/V_charge_detailAction.action").toString(),
				Integer.parseInt(start),Integer.parseInt(range),num);
		showPage.setPage(page);
		String page1=Page.getPage1(singleServletModel.getRequest(),
				new StringBuffer("action=").append(parm).append(ps.saveRequestParameters(v_charge_detail)).append("&go2page=edu/jt/exg/action/V_charge_detailAction.action").toString(),
        ...
        setShowPage(showPage);
		ps.setControlsValues(this);//控件初始化
		ps.readConditionsWrite2Controls(this,v_charge_detail);
		return parm;
	}
