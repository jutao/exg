...
import edu.jt.exg.logic.v_comment.InitPageControl;
...
import edu.jt.exg.logic.v_comment.V_commentUtility;



public class V_commentAction extends V_commentAttributeAction {
	...
	public String execute() throws Exception {
		...
		ps=V_commentUtility.getIocHandel().getBean("PageStatus",PageStatus.class);
	...
	private String doCommonInitQuery(String parm){
		...
		batisDao=V_commentUtility.getIocHandel().getBean("V_commentBatisDao",V_commentBatisDao.class);
		
		
		orderByMap.put(V_commentBatisDao.ID_ASC, null);//sqlserver必须排序否则分页会有问题
		
		
		
		
		v_commentList=batisDao.findByAll(orderByMap,start,range);
		
    	v_commentList=ps.wrapperRecorders(this,"show");
    	
    	
    	long num=batisDao.findByAllCount();
    	
        String page=Page.getPage(singleServletModel.getRequest(),
        		new StringBuffer("action=").append(parm).append("&go2page=edu/jt/exg/action/V_commentAction.action").toString(),
        ...
        setStart(start);
        setV_commentList(v_commentList);
	...
	private String doCommonQuery(String parm){
		...
		if(params!=null){
			...
			v_comment=ps.getQueryConditionsFromParams(this, params);
		}else{
			ps.setRecordsRangeAndOrderByRules(this);
			v_comment=ps.getQueryConditionsFromForm(this);
		}
		...
		v_commentList=batisDao.finder(orderByMap,start,range,v_comment);
		
		v_commentList=ps.wrapperRecorders(this,"show");
		
		
		long num=batisDao.finderCount(v_comment);
		
		String page=Page.getPage(singleServletModel.getRequest(),
				new StringBuffer("action=").append(parm).append(ps.saveRequestParameters(v_comment)).append("&go2page=edu/jt/exg/action/V_commentAction.action").toString(),
				Integer.parseInt(start),Integer.parseInt(range),num);
		showPage.setPage(page);
		String page1=Page.getPage1(singleServletModel.getRequest(),
				new StringBuffer("action=").append(parm).append(ps.saveRequestParameters(v_comment)).append("&go2page=edu/jt/exg/action/V_commentAction.action").toString(),
        ...
        setShowPage(showPage);
		ps.setControlsValues(this);//控件初始化
		ps.readConditionsWrite2Controls(this,v_comment);
		return parm;
	}
