...
import edu.jt.exg.logic.mgr_user.PageStatus;




public class Mgr_userAction extends Mgr_userAttributeAction {
	...
	public String execute() throws Exception {
		...
		ps=Mgr_userUtility.getIocHandel().getBean("PageStatus",PageStatus.class);
	...
	private String doCommonInitQuery(String parm){
		...
		batisDao=Mgr_userUtility.getIocHandel().getBean("Mgr_userBatisDao",Mgr_userBatisDao.class);
		
		
		orderByMap.put(Mgr_userBatisDao.ID_ASC, null);//sqlserver必须排序否则分页会有问题
		
		
		
		
		mgr_userList=batisDao.findByAll();
		
    	mgr_userList=ps.wrapperRecorders(this,"show");
    	
    	
    	long num=batisDao.findByAllCount();
    	
        String page=Page.getPage(singleServletModel.getRequest(),
        		new StringBuffer("action=").append(parm).append("&go2page=Mgr_userAction.action").toString(),
	...
	private String doCommonQuery(String parm){
		...
		if(params!=null){
			...
			mgr_user=ps.getQueryConditionsFromParams(this, params);
		}else{
			ps.setRecordsRangeAndOrderByRules(this);
			mgr_user=ps.getQueryConditionsFromForm(this);
		}
		...
		mgr_userList=batisDao.finder(mgr_user);
		
		mgr_userList=ps.wrapperRecorders(this,"show");
		
		
		long num=batisDao.finderCount(mgr_user);
		
		String page=Page.getPage(singleServletModel.getRequest(),
				new StringBuffer("action=").append(parm).append(ps.saveRequestParameters(mgr_user)).append("&go2page=Mgr_userAction.action").toString(),
				Integer.parseInt(start),Integer.parseInt(range),num);
		showPage.setPage(page);
		String page1=Page.getPage1(singleServletModel.getRequest(),
				new StringBuffer("action=").append(parm).append(ps.saveRequestParameters(mgr_user)).append("&go2page=Mgr_userAction.action").toString(),
        ...
        setShowPage(showPage);
		ps.setControlsValues(this);//控件初始化
		ps.readConditionsWrite2Controls(this,mgr_user);
		return parm;
	}
	...
	private String doDelete(){
		...
		batisDao=Mgr_userUtility.getIocHandel().getBean("Mgr_userBatisDao",Mgr_userBatisDao.class);
		Mgr_userBean mgr_user=batisDao.findByKey(id);
